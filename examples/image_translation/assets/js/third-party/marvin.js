...
MarvinColorModelConverter.rgbToBinary = function (img, threshold) {
  const resultImage = new MarvinImage(img.getWidth(), img.getHeight(), MarvinImage.COLOR_MODEL_BINARY);

  for (let y = 0; y < img.getHeight(); y++) {
    for (let x = 0; x < img.getWidth(); x++) {
      const gray = Math.ceil(((img.getIntComponent0(x, y) * 0.3) + (img.getIntComponent1(x, y) * 0.59) + (img.getIntComponent2(x, y) * 0.11)));
...
MarvinColorModelConverter.binaryToRgb = function (img) {
  const resultImage = new MarvinImage(img.getWidth(), img.getHeight(), MarvinImage.COLOR_MODEL_RGB);
...
MarvinImage.prototype.clone = function () {
  const image = new MarvinImage(this.getWidth(), this.getHeight(), this.colorModel);
...
MarvinMath.getTrueMatrix = function (rows, cols) {
  const ret = MarvinJSUtils.createMatrix2D(rows, cols);
...
MarvinMath.scaleMatrix = function (matrix, scale) {
  const ret = MarvinJSUtils.createMatrix2D(matrix.length, matrix.length);
...
GaussianBlur.prototype.getGaussianKernel = function () {
  const l_matrix = MarvinJSUtils.createMatrix2D((this.radius * 2) + 1, (this.radius * 2) + 1);
...
AlphaBoundary.prototype.process = function
  ...
  const neighborhood = getAttribute('radius');
...
BrightnessAndContrast.prototype.process = function
  ...
  let r, 
    g,
    b;
  const l_brightness = this.getAttribute('brightness');
...
ColorChannel.prototype.process = function
  ...
  const vr = this.getAttribute('red');
  const vg = this.getAttribute('green');
  const vb = this.getAttribute('blue');
...
ThresholdingNeighborhood.prototype.process = function
  ...
  const neighborhoodSide = getAttribute('neighborhoodSide');
  const samplingPixelDistance = getAttribute('samplingPixelDistance');
  const thresholdPercentageOfAverage = getAttribute('thresholdPercentageOfAverage');
...
Moravec.prototype.process = function
  ...
  const matrixSize = this.getAttribute('matrixSize');
  const threshold = this.getAttribute('threshold');

  const tempImage = new MarvinImage(imageIn.getWidth(), imageIn.getHeight());
  Marvin.grayScale(imageIn, tempImage);

  const cornernessMap = MarvinJSUtils.createMatrix2D(tempImage.getWidth(), tempImage.getHeight(), 0);
  const cornernessMapOut = MarvinJSUtils.createMatrix2D(tempImage.getWidth(), tempImage.getHeight(), 0);
...
BoundaryFill.prototype.process = function
  ...
  const tileImage = this.getAttribute('tile');
  this.threshold = this.getAttribute('threshold');
  ...
  const fillMask = MarvinJSUtils.createMatrix2D(imgOut.getWidth(), imgOut.getHeight, false);
  fillMask[x][y] = true;
...
BoundaryFill.prototype.match = function (image, x, y, targetRed, targetGreen, targetBlue, threshold) {
  const diff = Math.abs(image.getIntComponent0(x, y) - targetRed) + Math.abs(image.getIntComponent1(x, y) - targetGreen) + Math.abs(image.getIntComponent2(x, y) - targetBlue);
...
FloodfillSegmentation.prototype.process = function
  ...
  if (attributesOut != null) {
    const returnType = this.getAttribute('returnType');
...
Scale.prototype.process = function
  ...
  if (!previewMode) {
    const width = imageIn.getWidth();
    const height = imageIn.getHeight();
    const newWidth = this.getAttribute('newWidth');
    const newHeight = this.getAttribute('newHeight');
...
MarvinPoint.prototype.getY = function () {
  return this.y;
}; const marvinLoadPluginMethods = function (callback) {
