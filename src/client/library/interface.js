...
class Interface {
    ...
    addLoopEventSpacer () {
        if (this.noOfSpacers === 0) {
            ...
            let events = this.app.find('#events .event')
    ...
    setControls (partInstance) {
        ...
        this.controls.find('.previousTrack').click((event) => {
            let previousPart = new Part(CoreParts[0].partData, CoreParts[0].partName)
            previousPart.start()
            .then(() => {
        ...
        this.controls.find('.nextTrack').click((event) => {
            let nextPart = new Part(CoreParts[1].partData, CoreParts[1].partName)
            nextPart.start()
            .then(() => {
                partInstance.enable(false)
